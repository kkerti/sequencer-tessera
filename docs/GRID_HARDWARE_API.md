...
### Backlight
Without setting `lcd_set_backlight(255)` on the screen "setup" event, we can not see any visuals.
...
```
- `draw_text_fast` only supports text size values in the increments of 8, so 8, 16, 24, 32, etc.
...
|---|---|---|---|
| `self:element_index([n])` | `bnu` | signed int | The element's logical index. On 16-button modules: `0..15`, top-left to bottom-right, row-major. PBF4: `0..11`. |
...
self:element_index()        -- 0..15 (BU16/EN16) or 0..11 (PBF4)
