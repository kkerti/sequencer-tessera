...
src/
  ...
  persist.lua       save/load engine state to disk (Lua chunk format).
                    ...
                    (autoload) and the small-button LOAD/SAVE arm chords
                    (small buttons 3/4).
  ...
  controls.lua      VSN1 UI: 6-mode EDIT screen (NOTE VEL GATE MUTE STEP LASTSTEP) + LASTSTEP/swing row; scale lives in the NOTE row
...
| **mode** | The currently-edited focus (NOTE/VEL/GATE/MUTE/STEP/LASTSTEP). Selected by VSN1 keyswitches 1..6. DUR is reached as SHIFT+endless in GATE focus and snaps to the musical ladder `{6, 12, 24, 36, 48, 72}` (16th→dotted-half @ 24 PPQN) pulses. SHIFT+turn in NOTE/VEL/GATE focus coarsens the increment to ±12 (octave / loud-soft / long-short jumps) — EXCEPT NOTE, whose SHIFT is repurposed: **SHIFT+turn in NOTE cycles the selected track's quantization scale** (scale is no longer a mode; its name stays visible in the NOTE row). STEP focus drives the endless encoder to move the selected step, so a single-VSN1 setup can walk through steps before editing their params. **RANDOMIZE**: hold keyswitch 7 (header shows `RNDM`, same orange style as the LOAD/SAVE flags), then press a param mode — NOTE/VEL/GATE/MUTE randomize that parameter across all steps `1..lastStep` of the selected track. With SHIFT latched, RANDOM+GATE randomizes `dur` (from the musical ladder) instead of `gate`; RANDOM pitch is full-range (playback scale is an after-thought that doesn't rewrite stored pitches, so it is left out of randomization). STEP/LASTSTEP presses under RANDOM are not randomizable. The VSN1 screen and EN16 LEDs use no per-mode hue: highlight is brightness-only on the active param row, and EN16 LED brightness reflects the focused parameter's value per visible step. The selected step is rendered at max brightness; muted steps are red on EN16. |
