...
Spawns the Lua sequencer as a coprocess and wires it to virtual MIDI ports:
  ...
  - the Lua process's stdout  -> MIDI notes OUT           (ON / OFF lines)

Usage:
    python3 tools/bridge.py --lua "lua proto/term/main.lua"

Then in Ableton: enable the "SequencerClockIn" output (send MIDI clock to it),
and record/monitor from "SequencerNotesOut". Set each Ableton track's channel to
match the sequencer track (track 1 -> ch 1, track 2 -> ch 2).
...
def proc_to_midi_out(proc, port_name):
    """The Lua process's stdout (ON/OFF lines) -> MIDI notes out."""
...
def main():
    ...
    ap.add_argument("--in-port", default="SequencerClockIn")
    ap.add_argument("--out-port", default="SequencerNotesOut")
    args = ap.parse_args()
