...
-- Engine handle (for tracks[]). The 14 read/write accessors controls needs
-- are defined inline below so the device can lazy-load this file without
-- pulling in the full host-only authoring surface (~40 KB). The bit layout
-- is duplicated from sequencer.lua / authoring.lua intentionally; it is a
-- documented public on-disk contract that cannot drift silently.
--
-- Engine-side methods that controls also calls (Track.setDirection,
-- Track.setLoopStart / setLoopEnd, Track.clearLoopStart / clearLoopEnd) live
-- in sequencer.lua and are reached via Seq.Track.
...
-- Local Step accessor table. Only the codes controls reads/writes today
-- (pitch / velocity / duration / gate / active). Asserts dropped on the
-- runtime path; controls only feeds clamped values from M.edit / M.toggle.
local Step = {}
function Step.getPitch(s)    return floor(s / P_PITCH) % 128 end
function Step.getVelocity(s) return floor(s / P_VEL)   % 128 end
function Step.getDuration(s) return floor(s / P_DUR)   % 128 end
function Step.getGate(s)     return floor(s / P_GATE)  % 128 end
function Step.getActive(s)   return floor(s / P_ACT)   % 2 == 1 end
function Step.setPitch(s, v)    return pack7(s, v, P_PITCH) end
function Step.setVelocity(s, v) return pack7(s, v, P_VEL)   end
function Step.setDuration(s, v) return pack7(s, v, P_DUR)   end
function Step.setGate(s, v)     return pack7(s, v, P_GATE)  end
function Step.setActive(s, v)   return packBit(s, v, P_ACT) end

-- Local Track read accessors. Walk the patterns array directly; this is the
-- same logic as authoring.lua's trackComputeStepCount / trackGetStepAtFlat
-- with no asserts on the hot path. setStep mutates the matching pattern's
-- step in place.
function Track.getStepCount(t)
    local n = 0
    for i = 1, t.patternCount do n = n + t.patterns[i].stepCount end
    return n
end
function Track.getPatternCount(t) return t.patternCount end
function Track.getStep(t, idx)
    local off = 0
    for i = 1, t.patternCount do
        local p = t.patterns[i]
        if idx <= off + p.stepCount then return p.steps[idx - off] end
        off = off + p.stepCount
    end
end
function Track.setStep(t, idx, s)
    local off = 0
    for i = 1, t.patternCount do
        local p = t.patterns[i]
        if idx <= off + p.stepCount then
            p.steps[idx - off] = s
            return
        end
        off = off + p.stepCount
